...
namespace Entity
{
	void LuaPathFind::updatePath(float x, float y, float tx, float ty)
	{
		...
		{
			AStar::Location loc = m_path[m_path.size() - 2];
			m_nextStepX = std::get<0>(loc) * TILE_SIZE + TILE_SIZE / 2;
			m_nextStepY = std::get<1>(loc) * TILE_SIZE + TILE_SIZE / 2;
			m_pathSize = static_cast<float>(m_path.size());
		}
	}
	...
	static float g_TileSize = TILE_SIZE;
	...
	ScriptComponent::ScriptComponent(nlohmann::json json)
	{
		...
		getGlobalNamespace(LuaState)
			...
			.addVariable("TileSize", &g_TileSize, false)
			.endNamespace()
		...
		luaL_dofile(LuaState, std::string("res/entities/scripts/" + path).c_str());
