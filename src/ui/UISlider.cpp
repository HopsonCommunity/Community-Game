...
namespace UI
{
		...
		, m_sliderState(0)
	{
		...
		m_label.setPosition({ static_cast<float>(rect.left) + rect.width / 2, static_cast<float>(rect.top) + rect.height / 4 });

		m_bgSprite.setTexture(textures[3]);
		m_bgSprite.setPosition(static_cast<float>(m_rect.left), static_cast<float>(m_rect.top));
		m_bgSprite.setScale(static_cast<float>(m_rect.width) / 56.0f, static_cast<float>(m_rect.height) / 14.0f);

		m_scaleX = static_cast<float>(rect.width) / 56.0f;
	...
	void UISlider::setTexture(int mode)
	{
		...
		m_sliderSprite.setScale(static_cast<float>(m_rect.width) / 56.0f, static_cast<float>(m_rect.height) / 14.0f);
		m_currentMode = mode;
	}
	...
	void UISlider::determineSliderState(int x)
	{
		...
		{
			float xNorm = static_cast<float>(x) / m_scaleX;
			xNorm -= 3;
